...
function currentExercise() {
  return $("exercise").value.trim() || selectedExercise;
}
...
function isPR(session) {
  ...
  const maxPrior = Math.max(...prior.map(s => s.weight));
  return session.weight > maxPrior;
}
