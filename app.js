...
function currentExercise() {
  return $("exercise") ? $("exercise").value.trim() || selectedExercise : selectedExercise;
}
...
function updateGlobalStats() {
  ...
  if (data.length) {
    const firstDate = data.reduce((a,b) => a.date < b.date ? a : b).date;
    $("stat-days").textContent = daysBetween(firstDate) + 1;
  } else {
...
function renderHome() {
  ...
  const cards = GROUPS.filter(g => g.id !== "home").map(g => {
    ...
    const lastDate = sessions.length
      ? sessions.sort((a,b)=>b.date.localeCompare(a.date))[0].date : null;
    return `
      ...
      <section class="card">
        ...
        <div class="row">
          ...
          <div class="field"><label>Sets</label>
            <input id="sets" type="number" inputmode="numeric" placeholder="0"/></div>
        </div>
...
function renderChart(groupId) {
  ...
  if (chartMode === "volume") {
    const byDate = {};
    points.forEach(s => {
      byDate[s.date] = (byDate[s.date] || 0) + s.weight * s.reps * s.sets;
    });
    labels = Object.keys(byDate).sort();
    values = labels.map(d => byDate[d]);
  } else {
    // weight — max weight per date
    const byDate = {};
    points.forEach(s => {
...
function drawChart(labels, values) {
  ...
  const accentFill  = chartMode === "volume" ? "rgba(167,139,250,0.35)" : "rgba(59,130,246,0.35)";
  const dotStroke   = chartMode === "volume" ? "#c4b5fd" : "#38bdf8";
  ...
  values.forEach((v,i) => {
    ...
    ctx.strokeStyle = dotStroke; ctx.lineWidth = 2; ctx.stroke();
  });
